...
export class GPXStatisticsTree {
    ...
    constructor(element: GPXFile | Track) {
        const newStats: { [key: number]: GPXStatisticsTree | GPXStatistics } = {};
        if (element instanceof GPXFile) {
            this.level = ListLevel.FILE;
            element.children.forEach((child: Track, index: number) => {
                newStats[index] = new GPXStatisticsTree(child);
            });
        } else {
            this.level = ListLevel.TRACK;
            element.children.forEach((child: TrackSegment, index: number) => {
                newStats[index] = child.getStatistics();
            });
        }
        ...
        db.patches
            ...
            .then((keys) => {
                if (keys.length === 0) {
                    ...
                    // Cast keys to number, assuming they are numeric indices
                    const minKey = keys[0] as number;
                    const maxKey = keys[keys.length - 1] as number;
                    return { min: minKey, max: maxKey + 1 };
                }
            }),
    { min: 0, max: 0 }
);
...
function getChangedFileIds(patch: Patch[]): string[] {
    ...
    for (const p of patch) {
        // Ensure the first path segment (fileId) is treated as a string
        if (p.path.length > 0) {
            changedFileIds.add(String(p.path[0]));
        }
    }
        ...
        applyGlobal((draft) => {
            ...
            if (targetFile) {
                if (target instanceof ListFileItem) {
                    (targetFile as GPXFile).replaceTracks(
                        0,
                        targetFile.trk.length - 1,
                        toMerge.trk
                    );
                    (targetFile as GPXFile).replaceWaypoints(
                        0,
                        targetFile.wpt.length - 1,
                        toMerge.wpt
                    );
                } else if (target instanceof ListTrackItem) {
                    const trackIndex = target.getTrackIndex();
                    (targetFile as GPXFile).replaceTrackSegments(trackIndex, 0, -1, toMerge.trkseg);
                } else if (target instanceof ListTrackSegmentItem) {
                    const trackIndex = target.getTrackIndex();
                    const segmentIndex = target.getSegmentIndex();
                    (targetFile as GPXFile).replaceTrackSegments(
                        trackIndex,
                        segmentIndex,
                        segmentIndex - 1,
                        toMerge.trkseg
            ...
            if (file) {
                if (level === ListLevel.FILE) {
                    ...
                    segmentIndices.forEach((segmentIndex) => {
                        points.push(
                            ...(file.trk?.[trackIndex]?.trkseg?.[segmentIndex]?.getTrackPoints() ??
                                [])
                        );
                    });
                } else if (level === ListLevel.WAYPOINTS) {
                    ...
                    points.push(
                        ...(waypointIndices
                            .map((waypointIndex) => file.wpt?.[waypointIndex])
                            .filter((wpt) => wpt) as Waypoint[])
                    );
                }
            }
        });
